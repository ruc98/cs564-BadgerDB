...
struct IndexMetaInfo{
   ...
   */
	...
	/**
	 * To indicate if root is a leaf or not
	 */
	bool 		isRootLeaf ;
};
...
struct NonLeafNodeInt{
	/**
	 * Total number of slots that have been filled
	 */
	int entries;
 ...
 * @param pid: Page id of leaf node being inserted into
 * @param key: Pointer to value being inserted
 * @param rid: Record Id of tuple in the relation
 * @param path: Vector containing all nodes visited in order prior to leaf
 */
 const void insertIntoLeaf(const PageId pid, const void* key, const RecordId rid, std::vector<PageId>& path);


 /**
  * Split the leaf into two siblings and insert <key,rid> pair
  * @param pid: Page id of leaf node being inserted into
  * @param key: Pointer to value being inserted
  * @param rid: Record Id of tuple in the relation
  * @param path: Vector containing all nodes visited in order prior to leaf
  */
const void splitLeaf(const PageId pid, const void* key, const RecordId rid, std::vector<PageId>&path);



/**
 * Search the nonleaf nodes for the right leaf to insert <key,rid> into
 * @param pid: Page id of leaf node being inserted into
 * @param key: Pointer to value being inserted
 * @param rid: Record Id of tuple in the relation
 * @param path: Vector containing all nodes visited in order prior to leaf
 */
const void searchNodes(const PageId pid, const void* key, const RecordId rid, std::vector<PageId>&path);


/**
 * Insert key and pointer to right subchild into existing non leaf nodes
 * @param pid Page id of node being inserted into
 * @param key: Int value being inserted
 * @param rightId: PageId of new right child
 * @param path: Vector containing all nodes visited prior
 */
 const void insertIntoNode(const PageId pid, int key, const PageId rightId, std::vector<PageId>& path);



 /**
  * Split node and insert key, right subchild pageid
	* @param pid: Page id of node to be split
  * @param key: Int value being inserted
  * @param rightId: PageId of new right child
  * @param path: Vector containing all nodes visited prior
  */
const void splitNode(PageId pid,int key, const PageId rightId, std::vector<PageId>&path);



/**
 * Search for leaf occurence of key
 * @param pid: Page id of node to search from
 * @param key: Int value being searched
 * @param cid: PageId of leaf
 */
const void searchKey(PageId pid,int key, PageId &cid);
